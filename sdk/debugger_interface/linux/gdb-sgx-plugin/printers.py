...
class StdTuplePrinter(object):
    ...
    class _Children(object):
        ...
        def __init__(self, val):
            self.val = val
            self.child_iter = iter(self.val["base_"].type.fields())
            self.count = 0
        ...
        def __next__(self):
            ...
            child = self.val["base_"][field_name]["value"]
            self.count += 1
...
def _value_of_pair_first(value):
    ...
    return _get_base_subobject(value, 0)["__first_"]
...
class StdSharedPointerPrinter(object):
    ...
    def to_string(self):
        ...
        if refcount != 0:
            ...
            usecount_str = read_from_memory(cntrl_addr + 8, 8)
            usecount = struct.unpack("Q", usecount_str)[0]
            weakcount_str = read_from_memory(cntrl_addr + 16, 8)
            weakcount = struct.unpack("Q", weakcount_str)[0]
            if usecount == 0:
    ...
    def __init__(self, val):
        ...
        if self.val.type.template_argument(0).code == gdb.TYPE_CODE_BOOL:
            ...
            self.capacity = _get_base_subobject(
                self.val["__end_cap_"])["__first_"] - begin
