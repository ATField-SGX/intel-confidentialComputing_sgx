...
IPPAPI(IppStatus, k1_ippsHashSqueeze_rmf, (Ipp8u* pMD, const int digestLen, IppsHashState_rmf* pState))
IPPAPI(IppStatus, l9_ippsHashSqueeze_rmf, (Ipp8u* pMD, const int digestLen, IppsHashState_rmf* pState))
IPPAPI(IppStatus, y8_ippsHashSqueeze_rmf, (Ipp8u* pMD, const int digestLen, IppsHashState_rmf* pState))

IPPFUN(IppStatus, sgx_disp_ippsHashSqueeze_rmf, (Ipp8u* pMD, const int digestLen, IppsHashState_rmf* pState))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsHashSqueeze_rmf(pMD, digestLen, pState);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsHashSqueeze_rmf(pMD, digestLen, pState);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsHashSqueeze_rmf(pMD, digestLen, pState);
    }
