...
IPPAPI(IppStatus, k1_ippsMLDSA_KeyGenBufferGetSize, (int* pSize, const IppsMLDSAState* pMLDSAState))
IPPAPI(IppStatus, l9_ippsMLDSA_KeyGenBufferGetSize, (int* pSize, const IppsMLDSAState* pMLDSAState))
IPPAPI(IppStatus, y8_ippsMLDSA_KeyGenBufferGetSize, (int* pSize, const IppsMLDSAState* pMLDSAState))

IPPFUN(IppStatus, sgx_disp_ippsMLDSA_KeyGenBufferGetSize, (int* pSize, const IppsMLDSAState* pMLDSAState))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsMLDSA_KeyGenBufferGetSize(pSize, pMLDSAState);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsMLDSA_KeyGenBufferGetSize(pSize, pMLDSAState);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsMLDSA_KeyGenBufferGetSize(pSize, pMLDSAState);
    }
