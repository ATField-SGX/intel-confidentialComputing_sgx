...
IPPAPI( const CryptoLibraryVersion*, k1_cryptoGetLibVersion, (void) )
IPPAPI( const CryptoLibraryVersion*, l9_cryptoGetLibVersion, (void) )
IPPAPI( const CryptoLibraryVersion*, y8_cryptoGetLibVersion, (void) )

IPPFUN( const CryptoLibraryVersion*, sgx_disp_cryptoGetLibVersion, (void) )
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_cryptoGetLibVersion();
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_cryptoGetLibVersion();
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_cryptoGetLibVersion();
    }
    return NULL;
}
