...
IPPAPI(IppStatus, k1_ippsXMSSPrivateKeyStateGetSize, (Ipp32s* pSize, IppsXMSSAlgo OIDAlgo))
IPPAPI(IppStatus, l9_ippsXMSSPrivateKeyStateGetSize, (Ipp32s* pSize, IppsXMSSAlgo OIDAlgo))
IPPAPI(IppStatus, y8_ippsXMSSPrivateKeyStateGetSize, (Ipp32s* pSize, IppsXMSSAlgo OIDAlgo))

IPPFUN(IppStatus, sgx_disp_ippsXMSSPrivateKeyStateGetSize, (Ipp32s* pSize, IppsXMSSAlgo OIDAlgo))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsXMSSPrivateKeyStateGetSize(pSize, OIDAlgo);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsXMSSPrivateKeyStateGetSize(pSize, OIDAlgo);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsXMSSPrivateKeyStateGetSize(pSize, OIDAlgo);
    }
