...
IPPAPI(IppStatus, k1_ippsMLKEM_DecapsBufferGetSize, (int* pSize, const IppsMLKEMState* pMLKEMCtx))
IPPAPI(IppStatus, l9_ippsMLKEM_DecapsBufferGetSize, (int* pSize, const IppsMLKEMState* pMLKEMCtx))
IPPAPI(IppStatus, y8_ippsMLKEM_DecapsBufferGetSize, (int* pSize, const IppsMLKEMState* pMLKEMCtx))

IPPFUN(IppStatus, sgx_disp_ippsMLKEM_DecapsBufferGetSize, (int* pSize, const IppsMLKEMState* pMLKEMCtx))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsMLKEM_DecapsBufferGetSize(pSize, pMLKEMCtx);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsMLKEM_DecapsBufferGetSize(pSize, pMLKEMCtx);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsMLKEM_DecapsBufferGetSize(pSize, pMLKEMCtx);
    }
