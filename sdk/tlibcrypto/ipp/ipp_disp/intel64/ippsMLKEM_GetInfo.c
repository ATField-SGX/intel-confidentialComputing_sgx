...
IPPAPI(IppStatus, k1_ippsMLKEM_GetInfo, (IppsMLKEMInfo* pInfo, IppsMLKEMParamSet schemeType))
IPPAPI(IppStatus, l9_ippsMLKEM_GetInfo, (IppsMLKEMInfo* pInfo, IppsMLKEMParamSet schemeType))
IPPAPI(IppStatus, y8_ippsMLKEM_GetInfo, (IppsMLKEMInfo* pInfo, IppsMLKEMParamSet schemeType))

IPPFUN(IppStatus, sgx_disp_ippsMLKEM_GetInfo, (IppsMLKEMInfo* pInfo, IppsMLKEMParamSet schemeType))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsMLKEM_GetInfo(pInfo, schemeType);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsMLKEM_GetInfo(pInfo, schemeType);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsMLKEM_GetInfo(pInfo, schemeType);
    }
