...
IPPAPI(IppStatus, k1_ippsHashDRBG_Uninstantiate, (IppsHashDRBGState* pDrbgCtx))
IPPAPI(IppStatus, l9_ippsHashDRBG_Uninstantiate, (IppsHashDRBGState* pDrbgCtx))
IPPAPI(IppStatus, y8_ippsHashDRBG_Uninstantiate, (IppsHashDRBGState* pDrbgCtx))

IPPFUN(IppStatus, sgx_disp_ippsHashDRBG_Uninstantiate, (IppsHashDRBGState* pDrbgCtx))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsHashDRBG_Uninstantiate(pDrbgCtx);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsHashDRBG_Uninstantiate(pDrbgCtx);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsHashDRBG_Uninstantiate(pDrbgCtx);
    }
