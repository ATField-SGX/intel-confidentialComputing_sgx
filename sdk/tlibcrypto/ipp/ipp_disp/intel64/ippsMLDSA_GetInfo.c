...
IPPAPI(IppStatus, k1_ippsMLDSA_GetInfo, (IppsMLDSAInfo* pInfo, IppsMLDSAParamSet schemeType))
IPPAPI(IppStatus, l9_ippsMLDSA_GetInfo, (IppsMLDSAInfo* pInfo, IppsMLDSAParamSet schemeType))
IPPAPI(IppStatus, y8_ippsMLDSA_GetInfo, (IppsMLDSAInfo* pInfo, IppsMLDSAParamSet schemeType))

IPPFUN(IppStatus, sgx_disp_ippsMLDSA_GetInfo, (IppsMLDSAInfo* pInfo, IppsMLDSAParamSet schemeType))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsMLDSA_GetInfo(pInfo, schemeType);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsMLDSA_GetInfo(pInfo, schemeType);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsMLDSA_GetInfo(pInfo, schemeType);
    }
