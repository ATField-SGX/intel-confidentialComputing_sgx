...
IPPAPI( IppStatus, k1_ippsHashMethodSet_SHAKE256, (IppsHashMethod* pMethod, int digestBitsize) )
IPPAPI( IppStatus, l9_ippsHashMethodSet_SHAKE256, (IppsHashMethod* pMethod, int digestBitsize) )
IPPAPI( IppStatus, y8_ippsHashMethodSet_SHAKE256, (IppsHashMethod* pMethod, int digestBitsize) )

IPPFUN( IppStatus, sgx_disp_ippsHashMethodSet_SHAKE256, (IppsHashMethod* pMethod, int digestBitsize) )
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsHashMethodSet_SHAKE256(pMethod, digestBitsize);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsHashMethodSet_SHAKE256(pMethod, digestBitsize);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsHashMethodSet_SHAKE256(pMethod, digestBitsize);
    }
