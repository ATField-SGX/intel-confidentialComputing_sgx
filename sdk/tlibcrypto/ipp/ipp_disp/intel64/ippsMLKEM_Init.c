...
IPPAPI(IppStatus, k1_ippsMLKEM_Init, (IppsMLKEMState* pMLKEMCtx, IppsMLKEMParamSet schemeType))
IPPAPI(IppStatus, l9_ippsMLKEM_Init, (IppsMLKEMState* pMLKEMCtx, IppsMLKEMParamSet schemeType))
IPPAPI(IppStatus, y8_ippsMLKEM_Init, (IppsMLKEMState* pMLKEMCtx, IppsMLKEMParamSet schemeType))

IPPFUN(IppStatus, sgx_disp_ippsMLKEM_Init, (IppsMLKEMState* pMLKEMCtx, IppsMLKEMParamSet schemeType))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsMLKEM_Init(pMLKEMCtx, schemeType);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsMLKEM_Init(pMLKEMCtx, schemeType);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsMLKEM_Init(pMLKEMCtx, schemeType);
    }
