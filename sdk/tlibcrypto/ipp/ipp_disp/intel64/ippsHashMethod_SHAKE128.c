...
IPPAPI( const IppsHashMethod*, k1_ippsHashMethod_SHAKE128, (int digestBitsize) )
IPPAPI( const IppsHashMethod*, l9_ippsHashMethod_SHAKE128, (int digestBitsize) )
IPPAPI( const IppsHashMethod*, y8_ippsHashMethod_SHAKE128, (int digestBitsize) )

IPPFUN( const IppsHashMethod*, sgx_disp_ippsHashMethod_SHAKE128, (int digestBitsize) )
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsHashMethod_SHAKE128(digestBitsize);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsHashMethod_SHAKE128(digestBitsize);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsHashMethod_SHAKE128(digestBitsize);
    }
    return NULL;
}
