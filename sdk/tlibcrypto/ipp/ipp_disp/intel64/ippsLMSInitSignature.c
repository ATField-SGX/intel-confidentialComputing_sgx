...
IPPAPI(IppStatus, k1_ippsLMSInitSignature, (const IppsLMSAlgoType lmsType, IppsLMSSignatureState* pSign))
IPPAPI(IppStatus, l9_ippsLMSInitSignature, (const IppsLMSAlgoType lmsType, IppsLMSSignatureState* pSign))
IPPAPI(IppStatus, y8_ippsLMSInitSignature, (const IppsLMSAlgoType lmsType, IppsLMSSignatureState* pSign))

IPPFUN(IppStatus, sgx_disp_ippsLMSInitSignature, (const IppsLMSAlgoType lmsType, IppsLMSSignatureState* pSign))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsLMSInitSignature(lmsType, pSign);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsLMSInitSignature(lmsType, pSign);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsLMSInitSignature(lmsType, pSign);
    }
