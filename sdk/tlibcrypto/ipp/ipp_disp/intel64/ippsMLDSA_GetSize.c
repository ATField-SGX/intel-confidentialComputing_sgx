...
IPPAPI(IppStatus, k1_ippsMLDSA_GetSize, (int* pSize))
IPPAPI(IppStatus, l9_ippsMLDSA_GetSize, (int* pSize))
IPPAPI(IppStatus, y8_ippsMLDSA_GetSize, (int* pSize))

IPPFUN(IppStatus, sgx_disp_ippsMLDSA_GetSize, (int* pSize))
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsMLDSA_GetSize(pSize);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsMLDSA_GetSize(pSize);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsMLDSA_GetSize(pSize);
    }
