...
IPPAPI( const IppsHashMethod*, k1_ippsHashMethod_SHA512_256_NI, (void) )
IPPAPI( const IppsHashMethod*, l9_ippsHashMethod_SHA512_256_NI, (void) )
IPPAPI( const IppsHashMethod*, y8_ippsHashMethod_SHA512_256_NI, (void) )

IPPFUN( const IppsHashMethod*, sgx_disp_ippsHashMethod_SHA512_256_NI, (void) )
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsHashMethod_SHA512_256_NI();
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsHashMethod_SHA512_256_NI();
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsHashMethod_SHA512_256_NI();
    }
    return NULL;
}
