...
IPPAPI( const IppsHashMethod*, k1_ippsHashMethod_SHA512_TT, (void) )
IPPAPI( const IppsHashMethod*, l9_ippsHashMethod_SHA512_TT, (void) )
IPPAPI( const IppsHashMethod*, y8_ippsHashMethod_SHA512_TT, (void) )

IPPFUN( const IppsHashMethod*, sgx_disp_ippsHashMethod_SHA512_TT, (void) )
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsHashMethod_SHA512_TT();
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsHashMethod_SHA512_TT();
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsHashMethod_SHA512_TT();
    }
    return NULL;
}
