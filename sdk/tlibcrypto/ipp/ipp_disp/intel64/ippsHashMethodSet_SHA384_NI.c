...
IPPAPI( IppStatus, k1_ippsHashMethodSet_SHA384_NI, (IppsHashMethod* pMethod) )
IPPAPI( IppStatus, l9_ippsHashMethodSet_SHA384_NI, (IppsHashMethod* pMethod) )
IPPAPI( IppStatus, y8_ippsHashMethodSet_SHA384_NI, (IppsHashMethod* pMethod) )

IPPFUN( IppStatus, sgx_disp_ippsHashMethodSet_SHA384_NI, (IppsHashMethod* pMethod) )
{
    Ipp64u _features = ippcpGetEnabledCpuFeatures();

    if (AVX3I_FEATURES == (_features & AVX3I_FEATURES))
    {
        return k1_ippsHashMethodSet_SHA384_NI(pMethod);
    }
    if (ippCPUID_AVX2 == (_features & ippCPUID_AVX2))
    {
        return l9_ippsHashMethodSet_SHA384_NI(pMethod);
    }
    if (ippCPUID_SSE42 == (_features & ippCPUID_SSE42))
    {
        return y8_ippsHashMethodSet_SHA384_NI(pMethod);
    }
