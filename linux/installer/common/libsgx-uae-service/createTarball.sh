...
python3 ${SCRIPT_DIR}/gen_source.py --bom=BOMs/libsgx-uae-service.txt --deliverydir=${ROOT_DIR} --installdir=pkgroot/libsgx-uae-service
python3 ${SCRIPT_DIR}/gen_source.py --bom=BOMs/libsgx-uae-service-package.txt --deliverydir=${ROOT_DIR} --cleanup=false
python3 ${SCRIPT_DIR}/gen_source.py --bom=../licenses/BOM_license.txt --deliverydir=${ROOT_DIR} --cleanup=false

# Create the tarball
UAE_SERVICE_VER=$(awk '/UAE_SERVICE_VERSION/ {print $3}' ${ROOT_DIR}/common/inc/internal/se_version.h|sed 's/^\"\(.*\)\"$/\1/')
pushd ${INSTALL_PATH} &> /dev/null
sed -i "s/\(UAE_SERVICE_VER=\).*/\1${UAE_SERVICE_VER}/" Makefile
